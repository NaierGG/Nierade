...
export async function POST(request: NextRequest) {
  ...
  const limit = await checkRateLimit(getKey("auth:signup", ip), RATE_LIMIT_MAX, RATE_LIMIT_WINDOW_MS);
  if (!limit.ok) {
  ...
  if (existing) {
    return NextResponse.json(
      { ok: false, error: { code: "EMAIL_IN_USE", message: "Email is already in use." } },
      { status: 409 }
    );
  }
